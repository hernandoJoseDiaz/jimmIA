...
def main():
    ...
    if os.path.isfile(path):
        ...
        fennn = output_chess_board(chessboard, file_name = os.path.join(out_dir, os.path.basename(path).split('.')[0] + ".svg"), lichess = True)
        print(fennn)
        st = Stockfish("C:/Users/herna/Downloads/stockfish-windows-x86-64-avx2/stockfish/stockfish-windows-x86-64-avx2")
        # Configura la fuerza del motor (opcional)
        st.set_skill_level(20)  # Nivel máximo 20

        # Define la posición con el FEN
        st.set_fen_position(fennn)

        # Obtiene la mejor jugada
        best_move = st.get_best_move()
        print(f"La mejor jugada es: {best_move}")
